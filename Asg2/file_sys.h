...
class inode_state {
    ...
    public:
        ...
        const string& set_prompt(const string& new_prompt);
        inode_ptr get_cwd();
        inode_ptr& get_root();
        vector<string> get_directory();
        void cd_push(string& dirname);
        void cd_pop();
        void set_cwd(inode_ptr curr_dir);
};
...
class base_file {
    ...
    public:
        ...
        virtual inode_ptr 
            mkdir (const string& dirname, inode_state& state) 
                = 0;
        virtual inode_ptr mkfile (const string& filename) = 0;
        virtual void print_dir () = 0;
        virtual void recursive_print_dir () = 0;
        virtual inode_ptr inode_from_name(const string& value) = 0;
        virtual void 
            add_map_pair(const string& dirname, inode_ptr i_ptr) 
                = 0;
...
class plain_file: public base_file {
    ...
    public:
        ...
        virtual void recursive_print_dir () override;
        virtual inode_ptr inode_from_name(const string& value) override;
        virtual void 
            add_map_pair(const string& dirname, inode_ptr i_ptr) 
                override;
...
class directory: public base_file {
    private:
        // Must be a map, not unordered_map, 
        // so printing is lexicographic
